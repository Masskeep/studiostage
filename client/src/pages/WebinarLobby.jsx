...
const WebinarLobby = () => {
  ...
  useEffect(() => {
    ...
    const initLobby = async () => {
      try {
        ...
        if (!res.ok) {
          if (mounted) {
            setMeetingError('This webinar does not exist or has expired.');
  ...
  const handleJoin = () => {
    ...
    navigate(`/webinar/${id}`, { state: { name: name.trim(), micOn, camOn } });
  };
  ...
  const handleAudioJoin = () => {
    ...
    navigate(`/webinar/${id}`, { state: { name: name.trim(), micOn, camOn: false } });
  };
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--bg-color)' }}>
        ...
        <main className="container" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ backgroundColor: 'var(--card-bg)', padding: '3rem', borderRadius: '20px', border: '1px solid var(--border-color)', textAlign: 'center', maxWidth: '400px' }}>
            ...
            <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem' }}>Webinar Not Found</h2>
            <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem' }}>{meetingError}</p>
            <button className="btn-primary" onClick={() => navigate(user ? '/webinars' : '/')} style={{ width: '100%' }}>
              Return Home
            </button>
          </div>
        </main>
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--bg-color)' }}>
        ...
        <main className="container" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{
            ...
            <h1 style={{ fontSize: '1.8rem', marginBottom: '0.5rem', fontFamily: 'var(--font-display)' }}>Join Webinar</h1>
            <p style={{ color: 'var(--text-secondary)', marginBottom: '2.5rem', fontSize: '0.95rem' }}>
              You are not logged in. How would you like to join this webinar?
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <button
                onClick={() => navigate('/auth', { state: { returnTo: `/webinar/${id}/lobby` } })}
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <main className="container" style={{ flex: 1, display: 'flex', paddingTop: '2rem', gap: '3rem', paddingBottom: '3rem' }}>
        ...
        <div style={{ flex: 1, backgroundColor: 'var(--card-bg)', borderRadius: '20px', padding: '2rem', border: '1px solid var(--border-color)', height: 'fit-content', boxShadow: '0 4px 16px rgba(0,0,0,0.04)' }}>
          <p style={{ color: 'var(--primary-purple)', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.08em', marginBottom: '0.5rem' }}>WEBINAR DETAILS</p>
          <h2 style={{ fontSize: '1.4rem', marginBottom: '0.25rem' }}>Webinar Room</h2>
